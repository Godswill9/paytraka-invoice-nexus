...
import { Company, Customer, InvoiceLineItem, SalesInvoice, SalesInvoiceRequest } from "@/types/api";
...
import { companyComplianceIssues, customerComplianceIssues, lineItemComplianceIssues } from "@/lib/compliance";
...
export function SalesInvoicesPage() {
  ...
  const { invoices, pagination, pager, loading, error, post, update, remove, getInvoice } = useInvoices();
  ...
  async function handleFirsSubmit(invoice: SalesInvoice) {
    const customerIssues = customerComplianceIssues(customerById.get(invoice.customer_id));
    const supplierIssues = companyComplianceIssues(company);
    const itemIssues = lineItemComplianceIssues(invoice.line_items ?? []);
    const issues = [...customerIssues.map((item) => `Customer: ${item}`), ...supplierIssues.map((item) => `Company: ${item}`), ...itemIssues];
    if (issues.length) {
      setSubmissionResponse(`Submission blocked. Complete: ${issues.join(", ")}.`);
      return;
    }
    try {
      const response = await submit({ invoice_id: invoice.id });
      setSubmissionResponse(typeof response?.data === "string" ? response.data : JSON.stringify(response?.data ?? response));
      await update(invoice.id, { status: "sent" } as Partial<SalesInvoiceRequest>);
      notifyDashboard("Invoice successfully sent to FIRS");
    } catch (requestError) {
      setSubmissionResponse(getApiErrorMessage(requestError, "FIRS submission failed."));
    }
  }
  ...
  return (
    <>
      ...
      {user?.firs_enabled !== 1 ? <ComplianceAlert tone="warning" title="FIRS transmission is not enabled" text="Add your company FIRS credentials in Settings → Compliance to transmit posted invoices." /> : null}
      {submissionResponse ? <ComplianceAlert tone="warning" title="FIRS submission response" text={submissionResponse} /> : null}
      {error ? <ComplianceAlert title="Unable to load invoices" text={error} /> : null}
      <FilterBar labels={["Date range", "Status", "Customer", "Invoice type"]} />
      <DataTable
        title="Recent Invoices"
        columns={["Invoice", "Customer", "Dates", "Amount", "Balance Due", "Status", "Actions"]}
        rows={invoices.map((invoice) => {
          ...
          return {
            ...
            Customer: <div className="min-w-0"><p className="break-words font-semibold">{invoice.customer_name ?? customer?.name ?? "Customer"}</p><p className="mt-1 text-xs text-[#757588]" title={invoice.customer_id}>ID: {customer?.public_id ? shortReference(customer.public_id) : shortReference(invoice.customer_id)}</p></div>,
            ...
            Amount: <b><CurrencyAmount amount={salesInvoiceTotal(invoice)} currency={invoice.currency} /></b>,
            "Balance Due": <b className={salesInvoiceBalance(invoice) > 0 ? "text-amber-700" : "text-green-700"}><CurrencyAmount amount={salesInvoiceBalance(invoice)} currency={invoice.currency} /></b>,
            ...
            Actions: rowActions(actions, invoice.invoice_number, [
              ...
              ...(!isPosted ? [{ label: "Edit invoice", icon: Edit3, onSelect: () => void handleEdit(invoice) }] : []),
              { label: firsEnabled ? "Submit to FIRS" : "FIRS not enabled", icon: Send, disabled: !firsEnabled || !isPosted || invoice.status === "sent", onSelect: () => void handleFirsSubmit(invoice) },
              { label: "Download PDF", icon: Download, onSelect: () => void handleDownload(invoice) },
              ...(!isPosted ? [{ label: "Delete invoice", icon: Trash2, tone: "danger" as const, onSelect: () => void handleDelete(invoice) }] : []),
            ]),
          };
        })}
        footer={loading ? "Loading invoices..." : `Showing ${invoices.length} of ${pagination?.total ?? invoices.length} invoices`}
        footerActions={<Pagination pagination={pagination} onPageChange={pager.setPage} />}
        loading={loading}
      />
      {selectedInvoice ? <InvoiceDetailsModal invoice={selectedInvoice} company={company} customer={customerById.get(selectedInvoice.customer_id)} logoUrl={user?.logo_url} onClose={() => setSelectedInvoice(null)} onDownload={() => printInvoicePdf(selectedInvoice.invoice_number)} /> : null}
...
function InvoiceDetailsModal({ invoice, company, customer, logoUrl, onClose, onDownload }: { invoice: SalesInvoice; company: Company | null; customer?: Customer; logoUrl?: string | null; onClose: () => void; onDownload: () => void }) {
  return (
    <div className="fixed inset-0 z-[90] flex items-end justify-center overflow-hidden bg-[#191C1E]/55 p-0 backdrop-blur-sm sm:items-center sm:p-4 print:static print:block print:bg-white print:p-0" role="dialog" aria-modal="true" aria-labelledby="invoice-details-title" onMouseDown={onClose}>
      <div className="flex max-h-[96dvh] w-full max-w-6xl flex-col overflow-hidden rounded-t-2xl bg-[#E9EDF2] shadow-2xl sm:rounded-2xl print:max-h-none print:max-w-none print:overflow-visible print:rounded-none print:bg-white print:shadow-none" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex shrink-0 items-center justify-between gap-4 border-b border-[#C5C4DA] bg-white p-3 sm:p-4 print:hidden">
          <div><h2 id="invoice-details-title" className="font-bold">{invoice.invoice_number}</h2><p className="text-xs text-[#757588]">Invoice preview</p></div>
          <div className="flex items-center gap-2">
            <Button onClick={onDownload}><Download className="h-4 w-4" /> Download PDF</Button>
            <button type="button" onClick={onClose} aria-label="Close invoice details" className="rounded-lg p-2 text-[#454557] hover:bg-[#F1F4F8]"><X className="h-5 w-5" /></button>
          </div>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-2 sm:p-5 print:overflow-visible print:p-0">
          <InvoiceDocument invoice={invoice} company={company} customer={customer} logoUrl={logoUrl} />
        </div>
      </div>
    </div>
  );
