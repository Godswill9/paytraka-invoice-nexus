...
export function DashboardHome() {
  ...
  const { invoices, pagination: invoicePagination, error: invoiceError } = useInvoices();
  const customerById = new Map(customers.map((customer) => [customer.id, customer]));
  const missingCustomerTin = customers.filter((customer) => !customer.tax_identification_number).length;
  const missingProductTax = products.filter((product) => product.tax_rate == null).length;
  ...
  return (
    <>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Outstanding" value={<CurrencyAmount amount={totalOutstanding} />} meta="From loaded invoices" tone="primary" />
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <div className="mt-5 space-y-3">
              {[
                ["Customer TIN Missing", `${missingCustomerTin} customers need TIN values`, missingCustomerTin > 0 ? "danger" : "success", "/dashboard/customers"],
                ["VAT Configurations", "7.5% standard rate available for invoice items", "success", "/dashboard/settings"],
                ["Product Tax Rates Missing", `${missingProductTax} products need tax rates`, missingProductTax > 0 ? "danger" : "success", "/dashboard/products"],
          ...
          <DataTable
            ...
            rows={invoices.slice(0, 5).map((invoice) => {
              ...
              return {
                ...
                Amount: <CurrencyAmount amount={invoiceTotal(invoice)} currency={invoice.currency} />,
                "Customer Status": <StatusBadge>{customer?.tax_identification_number ? "verified" : "TIN missing"}</StatusBadge>,
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2">
              {customers.slice(0, 4).map((customer) => (
                <Link key={customer.id} href="/dashboard/customers" className="rounded-xl border border-[#C5C4DA] bg-[#F7F9FB] p-4">
                  ...
                  <p className="mt-1 text-sm text-[#454557]">{customer.tax_identification_number ?? "TIN missing"}</p>
