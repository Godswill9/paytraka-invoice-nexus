...
function DirectoryPage({ type }: { type: "customers" | "suppliers" }) {
  ...
  return (
    <>
      ...
      <Card className="mb-6 p-4"><div className="flex min-w-0 flex-wrap gap-3">{(isCustomers ? ["All Customers", "Missing TIN", "Active", "Recent"] : ["All Suppliers", "Missing TIN", "Verified", "Recent"]).map((tab, index) => <button key={tab} className={`rounded-full px-4 py-2 text-sm font-bold ${index === 0 ? "bg-[#DADEFD] text-[#0001B1]" : "text-[#454557]"}`}>{tab}</button>)}<Button variant="secondary" className="w-full sm:ml-auto sm:w-auto"><Filter className="h-4 w-4" /> Advanced Filter</Button><Button variant="secondary" className="w-full sm:w-auto"><Download className="h-4 w-4" /> Export CSV</Button></div></Card>
